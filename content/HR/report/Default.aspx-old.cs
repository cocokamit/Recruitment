...
public partial class content_HR_report_Default : System.Web.UI.Page
{
    ...
    protected void access()
    {
        ...
        DataView dv;
        DataSet ds;
        string y = ddl_year.Text == "0" ? "" : " and YEAR(date_app)=" + ddl_year.Text;
        ...
        "where YEAR(date_app) = " + ddl_year.Text + " " +
        "group by source");
        ...
        dt = dbhelper.getdata("select count(*) cnt from applicant where [status]<>'Hired' and [status]<>'reject' and [status]<>'cancel'");
        l_total.Text = dt.Rows[0]["cnt"].ToString();

        dt = dbhelper.getdata("select COUNT(*) cnt from applicant where [status]<>'Hired' and [status]<>'reject' and [status]<>'cancel' and left(CONVERT(varchar,date_app,101),10) = left(CONVERT(varchar,GETDATE(),101),10)");
