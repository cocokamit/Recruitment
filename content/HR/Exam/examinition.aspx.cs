...
public partial class content_HR_Exam_examinition : System.Web.UI.Page
{
    ...
    protected void save(object sender, EventArgs e)
    {
        ...
        else
        {
            ...
            if (cnt > 0 && cnt < 2)
            {
                ...
                ScriptManager.RegisterClientScriptBlock(this, this.GetType(), "alertMessage", "alert('Record Saved Successfully'); window.location='Examinition'", true);
            }
    ...
    {
        string query = "Select * from Examinition where status='1' order by id desc";
    ...
    protected void add(object sender, EventArgs e)
    {
        if (checker())
        {
            stateclass a = new stateclass();
            a.a = txt_exam.Text;
            a.b = DateTime.Parse(txt_limit.Text).ToString(@"hh\:mm\:ss"); ;
            a.c = "1";
            a.d = txt_passing.Text;

            bol.Examinition(a);
            ScriptManager.RegisterClientScriptBlock(this, this.GetType(), "alertMessage", "alert('Record Updated Successfully'); window.location='Examinition'", true);
        }
    }
    ...
    protected void customize_view(object sender, EventArgs e)
    {
        ...
        {
            Response.Redirect("questionnaire?id=" + row.Cells[0].Text);
            //id = row.Cells[0].Text;
    ...
    protected void update_view(object sender, EventArgs e)
    {
        //wala ma apil ang pag save sa passing rage
        using (GridViewRow row = (GridViewRow)((LinkButton)sender).Parent.Parent)
        {
            modaledit.Style.Add("display", "block");
            id = row.Cells[0].Text;
            query = "select description,time_limit from Examinition where id=" + id + "";
            DataTable dt = new DataTable();
            dt = dbhelper.getdata(query);

            txt_typee.Text = dt.Rows[0]["description"].ToString();
            txt_t.Text = dt.Rows[0]["time_limit"].ToString();
            customize.Visible = false;
        }
    }
    ...
    {
        query = "update examinition set description='"+txt_typee.Text+"',time_limit='"+txt_t.Text+"' where id="+id+"";
        dbhelper.getdata(query);
    ...
    protected void delete(object sender, EventArgs e)
    {
        if (TextBox1.Text == "Yes")
        {
            ...
            {
                id = row.Cells[0].Text;
                query = "update examinition set status='2' where id=" + id + "";
                dbhelper.getdata(query);
                ScriptManager.RegisterClientScriptBlock(this, this.GetType(), "alertMessage", "alert('Record Deleted Successfully'); window.location='Examinition'", true);
            }
        }
    }
