...
public partial class form : System.Web.UI.Page
{
    ...
    {
        DataTable dt = dbhelper.getdata("select * from Jobs where id = " + Request.QueryString["app_id"].ToString());
        if (dt.Rows.Count == 0)
        ...
        else
        {
            ...
            hf_app_id.Value = Request.QueryString["app_id"].ToString();
        }
    }
        ...
        if (dt.Rows.Count == 0)
        {
            ...
            if (checkfeilds())
            {
                try
                {
                    ...
                    txt_fname.Text.Replace(" ", "").Replace("'", "''") + "_" + txt_lname.Text.Replace(" ", "").Replace("'", "''") + txt_resume.Text.Replace("'", "''") + "','For appointment','" + txtCaptcha.Text + "','" + ddl_gender.Text + "'," + hf_property.Value + ") set @appid = scope_identity(); " +
