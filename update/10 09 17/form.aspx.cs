...
public partial class form : System.Web.UI.Page
{
    ...
    {
        string path = "temp/" + Request.QueryString["app_id"].ToString() + "_" + Session["captcha"].ToString();
        ...
        filename.Text = fname;
        paraupload.Text = Request.QueryString["app_id"].ToString() + "_" + Session["captcha"].ToString();
    ...
    protected void click_save(object sender, EventArgs e)
    {
        if (checkfeilds())
        {
            ...
            query += "insert into applicant(date_app,name,present_add,tel_no,cell_no,email,dateofbirth,job_id,resume_file,status,captcha,gender) values " +
            "(getdate(),'" + txt_fname.Text.Trim() + ' ' + txt_mname.Text.Trim() + ' ' + txt_lname.Text.Trim() + "','" + present + "','" +
            txt_tel.Text + "','" + txt_cel.Text + "','" + txt_email.Text + "','" + txt_from.Text.Replace(" ", "") + "'," + Request.QueryString["app_id"].ToString() + ",'" +
            txt_fname.Text.Replace(" ", "") + "_" + txt_lname.Text.Replace(" ", "") + txt_resume.Text + "','For appointment','" + txtCaptcha.Text + "','"+ddl_gender.Text+"') set @appid = scope_identity(); " +

           "insert into Education(app_id,level,school,year,address,job_id)values(@appid,'Elementary','" + txt_elemschool.Text + "','" + ddl_elemmonth.Text + "-" + ddl_elemyear.Text + "','" + txt_elemaddress.Text + "'," + Request.QueryString["app_id"].ToString() + ") " +
           "insert into Education(app_id,level,school,year,address,job_id)values(@appid,'Secondary','" + txt_secondschool.Text + "','" + ddl_secondmonth.Text + "-" + ddl_secondyear.Text + "','" + txt_secondaddress.Text + "'," + Request.QueryString["app_id"].ToString() + ") " +
           "insert into Education(app_id,level,school,year,address,job_id)values(@appid,'College','" + txt_collegeschool.Text + "','" + ddl_collegemonth.Text + "-" + ddl_collegeyear.Text + "','" + txt_collegeaddress.Text + "'," + Request.QueryString["app_id"].ToString() + ") " +
           "insert into Education(app_id,level,school,year,address,job_id)values(@appid,'Vocational','" + txt_vocationalschool.Text + "','" + ddl_vocationalmonth.Text + "-" + ddl_vocationalyear.Text + "','" + txt_vocationaladdress.Text + "'," + Request.QueryString["app_id"].ToString() + ") ";
             
            DataTable employment = (DataTable)ViewState["governmentlicenses"];
            foreach (DataRow row in employment.Rows)
            {
                query += "insert into EmploymentHistory(app_id,company,position,fromto,job_id)values(@appid,'" + row["col_1"] + "','" + row["col_2"] + "','" + row["col_3"] + "'," + Request.QueryString["app_id"].ToString() + ") ";
            }
            ...
            {
                query += "insert into special_skills (app_id,skills,[date]) values (@appid,'" + row["col_1"] + "',GETDATE()) ";
            }
            ...
            //base64 = base64textarea.InnerText;
            //bytes = Convert.FromBase64String(base64);
            //nw = Directory.CreateDirectory(Server.MapPath("~/document/app_id_" + dt.Rows[0]["id"].ToString() + "/"));
            //using (System.IO.FileStream stream = new System.IO.FileStream(Server.MapPath("~/document/app_id_" + dt.Rows[0]["id"].ToString() + "/" + txt_fname.Text.Replace(" ", "") + "_" + txt_lname.Text.Replace(" ", "") + "." + txt_resume.Text), System.IO.FileMode.Create))
            //{
            //    stream.Write(bytes, 0, bytes.Length);
            //    stream.Flush();
            //}
